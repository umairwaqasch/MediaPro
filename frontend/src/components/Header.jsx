    ...
    <header className="border-b border-slate-200 dark:border-studio-800 bg-white/90 dark:bg-studio-900/90 backdrop-blur-md sticky top-0 z-40 px-4 sm:px-6 py-2.5 flex items-center justify-between transition-colors">
      ...
      <div className="flex items-center space-x-2">
        ...
        <button
          ...
          title="Click to open Task & Hardware Center (Ctrl+T)"
        >
          {isGpu ? (
        ...
        {/* AI Face Extractor Trigger */}
        {activeVideo && (
          <button
            onClick={onOpenFaceExtractor}
            className="p-1.5 rounded-lg text-emerald-500 hover:text-emerald-400 bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/30 transition flex items-center gap-1 font-semibold text-xs shadow-sm shadow-emerald-500/10"
            title="AI Unique Face Extractor & Best-Shot Gallery (Alt+F)"
          >
            <UserCheck className="w-4 h-4 text-emerald-400" />
            <span className="hidden sm:inline text-[11px] pr-0.5">AI Faces</span>
          </button>
        )}

        {/* Record Screen & Camera Trigger */}
        <button
          onClick={onOpenRecorder}
          className="p-1.5 rounded-lg text-rose-500 hover:text-rose-600 bg-rose-500/10 hover:bg-rose-500/20 border border-rose-500/30 transition flex items-center gap-1 font-semibold text-xs"
          title="Record Screen, Window & Webcam (Alt+R)"
        >
          <Video className="w-4 h-4" />
          <span className="hidden sm:inline text-[11px] pr-0.5">Record</span>
        </button>

        {/* Presets & Recipes Manager Trigger */}
        <button
          ...
          title="Export Preset Manager & Reusable Recipes (Ctrl+P)"
        >
          <Bookmark className="w-4 h-4 text-brand-500" />
        </button>

        {/* Action History Drawer Trigger */}
        <button
          onClick={onToggleHistory}
          className="p-1.5 rounded-lg text-slate-600 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white bg-slate-100 dark:bg-studio-800 hover:bg-slate-200 dark:hover:bg-studio-700 border border-slate-200 dark:border-studio-700 transition"
          title="Action History & Undo/Redo Timeline (Ctrl+H)"
        >
        ...
        <button
          ...
          title="Open Global Task Center & Background Jobs (Ctrl+T)"
        >
        ...
        <button
          ...
          title={activeStudioMode === 'image' ? 'Open Image Media Library (Ctrl+L)' : 'Open Video Library & Outputs (Ctrl+L)'}
          className="flex items-center space-x-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold text-white bg-gradient-to-r from-brand-600 to-brand-700 hover:from-brand-500 hover:to-brand-600 shadow-sm transition relative active:scale-95"
        >
          <Layers className="w-3.5 h-3.5" />
          <span>{activeStudioMode === 'image' ? 'Image Library' : 'Video Library'}</span>
          {outputCount > 0 && (
            <span className="min-w-[18px] h-[18px] px-1 rounded-full bg-brand-rose text-[10px] font-bold flex items-center justify-center text-white ml-0.5">
              {outputCount}
            </span>
          )}
        </button>
